...
namespace FlowCanvas {
    ...
    abstract public class FlowNode : Node, ISerializationCallbackReceiver {
			...
			if (clickedPort != null && e.type == EventType.MouseUp){
				...
				///ON CANVAS
					...
					if (dragDropMisses == graph.allNodes.Count && clickedPort != null){
						...
						if (zoomFactor == 1){ menu.ShowAsBrowser(string.Format("Add & Connect (Type of {0})", cachePort.type.FriendlyName() ), graph.baseNodeType ); }
						else { GraphEditorUtility.PostGUI += ()=> { menu.ShowAsContext(); }; }
						e.Use();
					}
				}
			}
		...
		void DoDropInConnectionMenu(Port contextPort, BinderConnection refBinder = null){
			...
			if (menu.GetItemCount() == 1){ EditorUtils.GetMenuItems(menu)[0].func(); }
			else { GraphEditorUtility.PostGUI += ()=> { menu.ShowAsContext(); }; }			
		}
			...
			if (contextPort.IsInputPort()){
				if (orderedOutputs != null){
					foreach (var _port in orderedOutputs.Where(p => BinderConnection.CanBeBound(p, contextPort, refBinder) )){
						var port = _port;
						menu.AddItem(new GUIContent(string.Format("From: '{0}'", port.name) ), false, ()=>
						{
							if (refBinder != null){
								refBinder.SetSource(port);
							} else {
			...
			if (contextPort.IsOutputPort()){
				if (orderedInputs != null){
					foreach (var _port in orderedInputs.Where(p => BinderConnection.CanBeBound(contextPort, p, refBinder) )){
						var port = _port;
						menu.AddItem(new GUIContent(string.Format("To: '{0}'", port.name) ), false, ()=>
						{
							if (refBinder != null){
								refBinder.SetTarget(port);
							} else {
			...
			//Or value tip
				...
				if ( port is ValueInput && !port.isConnected){
					...
					if ( port.type.IsValueType || port.type == typeof(Type) || port.type == typeof(string) || port.IsUnityObject() ){
						...
						rect.y = port.rect.y - size.y * 0.3f; //*0.3? something's wrong here. TODO: FIX
		...
		void DrawNodePorts(){
			...
			var anyValueInputsShown = false;
			var anyValueOutputsShown = false;
			...
			for (var i = 0; i < maxIndex; i++){
				...
				if (i <= orderedInputs.Length - 1){
					...
					if (port.IsFlowPort()){
						...
						anyValueInputsShown |= port.willDraw;
					}
				}
				...
				if (i <= orderedOutputs.Length - 1){
					...
					if (port.IsFlowPort()){
						...
						anyValueOutputsShown |= port.willDraw;
					}
				}
			...
			if (anyValueInputsShown || anyValueOutputsShown){
				GUILayout.BeginVertical(Styles.roundedBox);
				for (var i = 0; i < maxIndex; i++){

					//mirror ports
					if (i <= orderedInputs.Length - 1 && i <= orderedOutputs.Length - 1){
						var portIn = orderedInputs[i];
						var portOut = orderedOutputs[i];
						if (portIn.name == portOut.name && portIn.type == portOut.type && portIn.IsValuePort() && portOut.IsValuePort()){
							GUILayout.BeginHorizontal();
							DrawNodePort(portIn, portOut);
							GUILayout.EndHorizontal();
							continue;
						}
					}
