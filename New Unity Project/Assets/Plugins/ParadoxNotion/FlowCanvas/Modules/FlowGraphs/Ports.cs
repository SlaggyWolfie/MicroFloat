	...
	abstract public class Port{
		...
		internal Rect rect
		{
			get
			{
				...
				if (IsInputPort()){
					result.center = new Vector2(parent.rect.xMin - 5, parent.rect.y + (willDraw? posOffsetY : 18 ));
				}
				if (IsOutputPort()){
					result.center = new Vector2(parent.rect.xMax + 5, parent.rect.y + (willDraw? posOffsetY : 18 ));
				}
				return result;
			}
		}
		...
		internal void EnsureCachedGUIContent(){
			...
			displayName = name.SplitCamelCase().CapLength(18);
			if (IsFlowPort()){
				...
				var tooltip = string.Empty; //TODO: make tooltips
				Texture icon = null;
